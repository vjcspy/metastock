...
class StockTradingAnalysisHullma:
    ...
    def get_data(self):
        ...
        if current_trend == -1:
            beginning_last_up_trend_index = self._find_beginning_last_up_trend(hullma_data_state)
            if beginning_last_up_trend_index is not None:
                price_df = hullma.get_price_helper().get_df_after_date(beginning_last_up_trend_index)
                max_price = price_df['high'].max()
                change_percent = round((max_price / hullma.get_price_helper().get_df().iloc[0]['close'] - 1) * 100)
            index_from_beginning_down_trend = self._find_beginning_current_down_trend(hullma_data_state)
            day_in_down_trend = hullma_data_state.index.get_loc(index_from_beginning_down_trend)
    ...
    def _find_beginning_last_up_trend(self, df: pd.DataFrame, date_range=10):
        ...
        if current_trend == -1:
            for index, row in df.iterrows():
                ...
                if _day_trend == 0:
                    continue
                if _day_trend == -1 and total_date_range >= date_range and last_up_trend:
                    return index
                if _day_trend == 1:
                    last_up_trend = True
    ...
    def _calculate_state_hullma(self, hullma_data: pd.Series, window=4):
        ...
        hullma_data_ca_ca_avg = hullma_data_ca_ca.abs().rolling(window=window).mean().shift(1)
        ...
        condition_positive = (hullma_data_ca > 0) & (
                (hullma_data_ca_ca > 0.5 * hullma_data_ca_ca_avg) | (hullma_data_ca_abs > 0.5 * hullma_data_ca_avg))
