...
class PredefinedStrategyGenerator(StrategyGeneratorAbstract):
    ...
    def _load_input_strategy_v1(self, strategy_config):
        ...
        if strategy_manager().get_class_map().get(self.strategy_name) is not None:
            try:
                self.strategy_class = strategy_manager().get_class_map().get(strategy_config['name'])['class']
        ...
        for config in self.strategy_inputs:
            ...
            try:
                ...
                if res.status_code == 409:
                    ...
                    self.logger.info(
                            f"[green]OK[/green] generated for strategy '{self.strategy_name}' and input name '{config['data']['name']}'"
                    )
                else:
                    self.logger.warning(
